    ...
    for (y = 0; y < resampled_height; ++y)
    {
        ...
        for (x = 0; x < resampled_width; ++x)
        {
            ...
            for (c = 0; c < channels; ++c)
            {
                ...
                value += orig[base_index + channels] * (samplex)
                    * (1.0f - sampley);
                value += orig[base_index + width * channels] * (1.0f - samplex)
                    * (sampley);
        ...
        for (i = 0; i < mip_width; ++i)
        {
            for (c = 0; c < channels; ++c)
            {
                ...
                for (v = 0; v < v_block; ++v)
                    for (u = 0; u < u_block; ++u)
                    {
                        sum_value += orig[index + v * width * channels
                                          + u * channels];
                    }
...
float find_max_RGBE(unsigned char* image, int width, int height)
{
    ...
    for (i = width * height; i > 0; --i)
    {
        ...
        for (j = 0; j < 3; ++j)
        {
            if (img[j] * scale > max_val) { max_val = img[j] * scale; }
        }
        /* next pixel */
        img += 4;
    }
                    ...
                    int rescale_to_max)
{
    ...
    float scale = 1.0f;
    /* error check */
    if ((!image) || (width < 1) || (height < 1)) { return 0; }
